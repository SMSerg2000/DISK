...
| Version | Changes |
|---------|---------|
| **3.0.1** | Fix: NVMe self-test/error-log reads now use `IOCTL_STORAGE_QUERY_PROPERTY` (like health), with `STORAGE_PROTOCOL_COMMAND` as fallback — drivers that reject the latter (StorNVMe/RAID/VMD, error 87) can now read the logs. NVMe self-test **start** still needs the protocol command (unavailable on such drivers) and says so clearly |
